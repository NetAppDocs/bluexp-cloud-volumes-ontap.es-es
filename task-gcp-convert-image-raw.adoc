...
[role="lead"]
La imagen que se utilizará para implementar nuevas instancias, actualizaciones o utilizarse en imágenes existentes se compartirá con los clientes a través de https://["El sitio de soporte de NetApp (NSS)"^]. El resumen firmado y los certificados se podrán descargar a través del portal de NSS. Asegúrese de descargar el resumen y los certificados de la versión correcta que corresponde a la imagen compartida por el soporte de NetApp. Por ejemplo, 9.13.0 imágenes tendrán un resumen firmado de 9.13.0 y certificados disponibles en NSS.
...
NOTE: Consulte https://["Documento de Google Cloud sobre la exportación de una imagen"^] Para obtener más información sobre cómo exportar una imagen a través de Google Cloud.
