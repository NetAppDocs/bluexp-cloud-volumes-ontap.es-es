...
+
A partir de Cloud Volumes ONTAP 9.15.1, puede implementar instancias de equipos virtuales en zonas de disponibilidad únicas de almacenamiento redundante local (LRS). Esto también requiere que `Microsoft.Compute/VMOrchestratorZonalMultiFD` se active la función para su suscripción. Esta función facilita la puesta en marcha de pares de alta disponibilidad en la misma zona de disponibilidad. Consulte link:task-saz-feature.html["Activar VMOrchestratorZonalMultiFD para zonas de disponibilidad únicas"]y.link:task-azure-high-availability-mode.html["Habilitar el modo de alta disponibilidad en Azure"]
...
+
*** Para zonas de disponibilidad únicas, seleccione una región de Azure, una zona de disponibilidad, vnet y una subred. A partir de Cloud Volumes ONTAP 9.15.1, puede instalar instancias de VM en una sola zona de disponibilidad. Debe seleccionar una región que admita este modo. Si la región seleccionada por usted no soporta el despliegue zonal, entonces se sigue el modelo de despliegue no zonal anterior para LRS.
+
Para obtener más información sobre este modo de implementación y las zonas de Azure compatibles, consulte https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/["Documentación de Microsoft Azure: Documentación de Virtual Machine Scale Sets"^] y https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/["Sitio web de Microsoft Azure: Productos disponibles por región"^].
