...
Si crea una nueva versión de su clave, Cloud Volumes ONTAP utiliza automáticamente la última versión de clave, lo que garantiza que sus datos permanezcan seguros sin necesidad de actualizaciones manuales.

.Pasos
. Desplácese hasta el almacén de claves en el portal de Azure.
. Seleccione la tecla que desea girar.
. Haga clic en *Nueva versión* para generar una nueva versión de la clave.


Una vez que se crea una nueva versión, Cloud Volumes ONTAP detecta y utiliza automáticamente la última versión de la clave para el cifrado.
