...
. Consulte el resumen de los paquetes, donde se muestra la capacidad consumida, la capacidad total comprometida con anterioridad y la capacidad total de pago por uso (PAYGO).
+
...
** _Capacidad precomprometida total_ es la capacidad con licencia total (con su propia licencia (BYOL) o contrato de mercado) que ha adquirido a NetApp.
...
Si una licencia basada en capacidad expiró y ya no está en uso, puede eliminarla en cualquier momento.

Para obtener más información sobre lo que sucede con sus datos de Cloud Volumes ONTAP al vencimiento o eliminación de su licencia, consulte https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/FAQs_on_Cloud_Volumes_ONTAP_license_expiry["Este artículo de base de conocimientos (KB)"^] .
