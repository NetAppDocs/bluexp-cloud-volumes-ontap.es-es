...
[role="quick-margin-para"]
Debe habilitarse la organización de datos en niveles en un agregado para habilitarla en un volumen. Debe conocer los requisitos de los volúmenes nuevos y existentes. <<Ensuring that tiering is enabled on aggregates,Leer más>>.
...
+
BlueXP  crea un almacén de objetos para los datos inactivos cuando crea el sistema, <<Enabling data tiering after implementing the requirements,siempre que no haya problemas de conectividad ni de permisos>>. Después de eso, solo tiene que habilitar la organización de los datos en niveles en los volúmenes (y, en algunos casos, <<Ensuring that tiering is enabled on aggregates,en agregados>>).
