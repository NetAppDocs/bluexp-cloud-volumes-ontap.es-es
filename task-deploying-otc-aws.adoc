...
| Editar credenciales | Elija las credenciales de AWS y la suscripción al mercado para utilizar con este sistema Cloud Volumes ONTAP. Haga clic en *Agregar suscripción* para asociar las credenciales seleccionadas con una nueva suscripción al mercado de AWS. La suscripción puede ser por un contrato anual o para pagar por Cloud Volumes ONTAP a una tarifa por hora. Si adquirió una licencia directamente de NetApp (BYOL), no es necesaria una suscripción a AWS. https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-aws-accounts.html["Aprenda a añadir credenciales de AWS adicionales a BlueXP"^]. 
|===
+
El siguiente video muestra cómo asociar una suscripción de mercado de pago por uso a sus credenciales de AWS:
