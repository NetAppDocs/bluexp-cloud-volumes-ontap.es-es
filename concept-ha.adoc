...
Cloud Volumes ONTAP solo admite implementaciones de alta disponibilidad en zonas locales de AWS. A continuación se muestran configuraciones de ejemplo:
...
Cloud Volumes ONTAP solo admite estas configuraciones. La selección de cualquier otro tipo de disco o instancia EC2 en la configuración de la zona local de AWS para Cloud Volumes ONTAP puede provocar un fallo en la implementación. Consulte la documentación de AWS para obtener los detalles más recientes y completos de la link:https://aws.amazon.com/about-aws/global-infrastructure/localzones/features/?nc=sn&loc=2["EC2 Tipos de instancias en Zonas locales"^].
